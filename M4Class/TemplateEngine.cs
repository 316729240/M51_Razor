...
namespace MWMS
{
    ...
    public class TemplateEngine
    {
        ...
        public void renderSql(ref string html, ref SqlParameter[] p)
        {
        ...
        string getSqlLabel(string html)
        {
            ...
            SqlDataReader rs1 = Helper.Sql.ExecuteReader(sql, sql_p);
            #region 数据显示
            int index = 0;
        ...
        string getLabel(string html)
        {
            ...
            if (pageSize > 0)
            {
                ...
                if (debug == "true") return "调试：" + sql;
                rs1 = Helper.Sql.ExecuteReader(sql, sql_p);
        ...
        object getFunction(string fname, string parameter)
        {
            ...
            if (fname == "getSqlData")
            {
                ...
                return Helper.Sql.ExecuteArray(objv.ToString());
            }
            ...
            else if (fname == "write")
            {
                if (objv != null)
                {
                    HttpContext.Current.Response.Write(objv);
                }
            }
            else if (fname == "left")
            {
                ...
                if (objv != null)
                {
                    value = API.nohtml(getValue(p[0]).ToString());
                    value = API.GetString(value, Convert.ToInt32(getValue(p[1]))).String;
                }
                return value;
            }
            else if (fname == "split")
            ...
            {
                string f = getValue(p[0]).ToString();
                HttpContext.Current.Response.Redirect(f);
                return "";
            }
            else if (fname == "page")
            {
                ...
                if ((pageNo + 1) > s.Length || pageNo < 0) API.ERR404();
                return s[pageNo];
            }
        ...
        string compileFunction(Match m)
        {
            ...
            else
            {
                ...
                //    value = "<div class='m5_view' viewValue=\"" + HttpContext.Current.Server.HtmlEncode(key + "(" + viewValue + ")") + "\" >" + value + "</div>";
                    value = "<div class='m5_view' viewValue=\"" + HttpContext.Current.Server.HtmlEncode(key + "(" + viewValue + ")") + "\" ></div>"+value;
                }
