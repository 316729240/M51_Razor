$M.fileManage.manage = function (S) {
    ...
    var reloadDir = function () {
        ...
        item.attr("loadTag", "2");
        $M.comm([["fileManage.ajax.readDir", { path: item.attr("path")}], ["fileManage.ajax.readFiles", { path: item.attr("path")}]], function (json) {
            item.attr("loadTag", "1");
            item.clear();
            item.addItem(json[0]);
            fileList.clear();
    ...
    var createDir = function () {
        ...
        $M.prompt("目录名", function (value) {
            if (value == "") return;
            $M.comm("fileManage.ajax.createDir", { path: dirTree.selectedItem.attr("path"), name: value }, function (json) {
    ...
    var editDir = function () {
        ...
        $M.prompt("目录名", function (value) {
            ...
            $M.comm("fileManage.ajax.editDir", { path: dirTree.selectedItem.attr("path"), name: value }, function (json) {
        ...
        if (dirTree.selectedItem != null) {
            ...
            if (type == null) {
                $M.confirm("您确定要删除[" + dirTree.selectedItem.attr("text") + "]吗？", function () {
                    $M.comm("fileManage.ajax.delDir", { path: path }, function () { dirTree.selectedItem.remove(); dirTree.root.items[0].focus(); });
                });
            }
        }
    };
    ...
    var reload = function () {
        $M.comm("fileManage.ajax.readFiles", { path: dirTree.selectedItem.attr("path"), pageNo: _pageNo }, function (json) {
            fileList.clear();
    ...
    var delFile = function () {
        ...
        $M.confirm("您确定要删除所选文件吗？", function () {
            $M.comm("fileManage.ajax.delFile", { path: dirTree.selectedItem.attr("path"), files: files }, reload);
        });
    };
