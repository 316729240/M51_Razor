...
public class ajax : IHttpHandler {
    ...
    void readUserList(HttpContext context)
    {
        ...
        if (roleId > 0)
        {
            ...
            sql = "select A.id,A.uname,A.loginDateTime,B.roleId,ROW_NUMBER() Over(order by A.id) as rowNum from m_admin A  inner join admin_role B on A.id=B.userId where B.roleId=@roleId  and A.status=@status" + findName;
        }
        else
        {
            page.recordCount = (int)Sql.ExecuteScalar("select count(1) from m_admin where  status=@status" + findName, p);
            sql = "select id,uname,loginDateTime,ROW_NUMBER() Over(order by id) as rowNum from m_admin  where  status=@status" + findName;
        }
