$M.userManage.manage = function (S) {
    ...
    tab = mainTab.addItem({ text: "用户管理", name: "userManage", closeButton: true, onClose: function () { } });
    var _pageNo = 1;
    var _status = 1;
    ...
    var reloadClass = function () {
        $M.comm("templateManage.ajax.readTemplateViewClass", null, function (json) { roleGrid.clear(); roleGrid.addRow(json); });
    };
    ...
    var reload = function (pageno) {
        ...
        $M.comm("userManage.ajax.readUserList", { roleId: id, pageNo: pageno, status: _status, keyword: searchBox.val() }, function (json) {
            userList.clear();
    ...
    var delUser = function () {
        ...
        $M.confirm("您确定要删除所选用户吗？", function () {
            $M.comm("userManage.ajax.delUser", { id: id }, reloadClass);
        });
    };
    var menu1 = $(document.body).addControl({ xtype: "Menu", items: [
            { text: "新增", onClick: addUser },
		    { text: "删除", onClick: delUser },
		    { text: "-" },
    ...
    var delData = function () {
        ...
        $M.confirm("您确定要删除所选用户吗？", function () {
            var ids = getId();
            if (ids != "") {
                $M.comm("userManage.ajax.delUser", { ids: ids }, reload);
            }
        });
    };
...
//                stopButton.val(_status == 1 ? "停用" : "启用");
    ...
    $M.comm("userManage.ajax.readRole", null, function (json) { roleGrid.addRow({ id: 0, text: "全部" }); roleGrid.addRow(json); roleGrid.rows[0].focus(); });
    tab.focus();
};
