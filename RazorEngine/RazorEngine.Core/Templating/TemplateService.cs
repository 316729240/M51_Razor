namespace RazorEngine.Templating
{
    ...
    public class TemplateService : MarshalByRefObject, ITemplateService
    {
        ...
        {
            Compile(razorTemplate, null, name,true);
        }
        public void Compile(string razorTemplate, Type modelType, string name)
        {
            Compile(razorTemplate, modelType, name, true);
        }
        ...
        public virtual IEnumerable<ITemplate> CreateTemplates(IEnumerable<string> razorTemplates, IEnumerable<object> models, bool parallel = false)
        {
            ...
            //Contract.Requires(razorTemplates.Count() == models.Count(), "Expected the same number of model instances as the number of templates to be processed.");
        ...
        [Pure]
        public virtual Type CreateTemplateType(string razorTemplate,Type modeType)
        {
            return CreateTemplateType(razorTemplate, modeType, "",false);
        }
        /// <summary>
        /// Creates a <see cref="Type"/> that can be used to instantiate an instance of a template.
        /// </summary>
        ...
        public virtual Type CreateTemplateType(string razorTemplate, Type modelType,string name,bool flag)
        {
            ...
            if (File.Exists(assemblyPath) && !flag)
            {
                try
                {
                    ...
                    byte [] buff=System.IO.File.ReadAllBytes(assemblyPath);
        ...
        public virtual IEnumerable<ITemplate> GetTemplates(IEnumerable<string> razorTemplates, IEnumerable<string> names, bool parallel = false)
        {
            ...
            //Contract.Requires(razorTemplates.Count() == names.Count(), "Expected same number of cache names as string templates to be processed.");
