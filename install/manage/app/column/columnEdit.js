$M.column.columnEdit = function (S) {
    ...
    var obj = new $M.dialog({
        title: "栏目编辑",
        style: { width: "500px" },
        command: "column.ajax.columnEdit",
    ...
    var tab = obj.addControl({ xtype: "Tab", items: [{ text: "基本信息", "class": "form-horizontal" }, { text: "自定义变量" }, { text: "高级设置", "class": "form-horizontal"}] });
    ...
    var cList = tab.items[0].addControl([
        ...
        { xtype: "TextBox", name: "maxico", labelText: "栏目图片", labelWidth: 3 },
        { xtype: "SelectBox", name: "saveDataType", labelText: "栏目类型", labelWidth: 3 },
        { xtype: "SelectBox", name: "skinId", labelText: "模板", labelWidth: 3 },
    ...
    var editDirName = function () {
        $M.prompt("目录名称", function (text) {
            ...
            $M.comm("column.ajax.editDirName", { id: S.id, dirName: text }, function (json) {
                dirName.val(text);
                $M.dialog.reset(S.id);
            });
    ...
    tab.items[2].addControl({ xtype: "TextBox", name: "domainName", labelText: "绑定域名", labelWidth: 3 });
    var div=tab.items[2].append("<div class=\"formSep\"></div>");
    div.addControl({ xtype: "CheckBox", name: "inherit", items: [{ text: "继承设置", value: 1}], value: 1 });
    ...
    var commandList = [
            ["dataTypeList", null],
            ["templateList", { classId: S.id, type: 1}],
            ["templateList", { classId: S.id, type: 2}]
        ];
    if (S.id) commandList[commandList.length] = ["column.ajax.columnInfo", { id: S.id}];
    $M.comm(commandList, function (json) {
        cList[4].addItem(json[0]);
        cList[4].val(S.dataTypeId);
        if (json[1] != null) json[1].unshift({ text: "默认模板", value: 0 });
        else { json[1] = [{ text: "默认模板", value: 0}]; }
        if (json[2] != null) json[2].unshift({ text: "默认模板", value: 0 });
        else { json[2] = [{ text: "默认模板", value: 0}]; }
        cList[5].addItem(json[1]);
        cList[6].addItem(json[2]);
        if (S.id) {
            obj.form.val(json[3]);
            var xml = new $M.xml(json[3].custom);
