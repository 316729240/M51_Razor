$M.column.moduleEdit = function (S) {
    ...
    var obj = new $M.dialog({
        title: "模块编辑",
        style: { width: "500px" },
        command: "column.ajax.moduleEdit",
    ...
    var editDirName = function () {
        $M.prompt("目录名称", function (text) {
            ...
            $M.comm("column.ajax.editModuleDirName", { id: S.id, dirName: text }, function (json) {
                dirName.val(text);
    ...
    //tab.items[2].append("<div class=\"formSep\"></div>");
    var temp = tab.items[2].addControl({ xtype: "CheckBox", name: "saveRemoteImages", labelText: "保存图片", labelWidth: 3, items: [{ text: "", value: 1}], value: 1 });
    ...
    if (S.id) commandList[commandList.length] = ["column.ajax.moduleInfo", { id: S.id}];
