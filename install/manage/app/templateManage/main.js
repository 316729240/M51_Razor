$M.templateManage.manage = function (S) {
    ...
    tab = mainTab.addItem({ text: (S.u_webFAid==0?"PC":"手机")+ "模板管理", name: "templateManage", closeButton: true, onClose: function () { } });
    ...
    var delData = function () {
        ...
        $M.confirm("确定要删除所选模板？", function () {
            ...
            $M.comm("templateManage.ajax.delTemplate", { id: _id, classId: _classId, datatypeId: _datatypeId, typeId: typeId }, loadList);
        });
    };
    ...
    var toolBar = frame.items[1].addControl({
        xtype: "ToolBar", items: [[{ ico: "fa-plus", text: "创建模板", enabled: false, name: "createbutton" }, { ico: "fa-trash-o", text: "删除模板", enabled: false, name: "delbutton",
            ...
            , [{ text: "定位模板", onClick: function () {
                $M.prompt("网址",
                    function (value) {
                        $M.comm("templateManage.ajax.locateTemplate", { url: value, u_webFAid: S.u_webFAid }, function (json) {
    ...
    listGrid = frame.items[1].addControl({
        ...
        columns: [
                    ...
                    { text: "上次修改时间", name: "updateDate", width: 150 },
    ...
    loadList = function (back) {
        ...
        $M.comm("templateManage.ajax.templateList", { moduleId: moduleId, faId: faId, typeId: typeId, dataTypeId: dataTypeId, u_webFAid: S.u_webFAid },
                    function (json) {
                        listGrid.clear();
...
$M.templateManage.edit = function (S) {
    ...
    if ($M._templateManage_edit) {
        ...
        var editBox = null;
        $M._dataManage = mainTab.addItem({ text: (S.u_webFAid == 0 ? "PC" : "手机") + "模板编辑", closeButton: true, onClose: function () { $M._dataManage = null; } });
        ...
        var toolbar = frame.items[0].addControl({
            xtype: "ToolBar", items: [
                    [{ ico: "fa-save", name: "save", text: "保存", onClick: function () {
                        $M.comm("templateManage.ajax.saveTemplate", {
                            id: S.id,
                            title: S.title,
        ...
        editBox = frame.items[0].addControl({ xtype: "Editor", name: "u_content", sourceMode: true, style: { height: 300 }, dock: 2 });
        ...
        tree.root.addItem([{ ico: "fa-folder-o", text: "网站变量", type: 1 }, { ico: "fa-folder-o", text: "页面变量", type: 2 }, { ico: "fa-folder-o", text: "自定义标签", type: 3 }, { ico: "fa-folder-o", text: "自定义视图", type: 4}]);
        $M.comm([
                ["templateManage.ajax.readTemplateLable", { dataTypeId: S.datatypeId}],
                ["templateManage.ajax.readTemplate", { id: S.id, classId: S.classId, typeId: S.typeId, datatypeId: S.datatypeId, defaultFlag: S.defaultFlag, title: S.title,u_webFAid:S.u_webFAid}]
                ], function (json) {
                    for (var data in json[0].systemVariables) {
                        tree.root.items[0].addItem({ type: 11, ico: "fa-tag", text: json[0].systemVariables[data][0], value: data });
                    }
                    if (json[0].pageVariables) {
                        for (var i = 0; i < json[0].pageVariables.length; i++) {
                            ...
                            tree.root.items[1].addItem(json[0].pageVariables[i]);
                        }
                    }
...
$M.templateManage.label = function (back) {
    ...
    var toolbar = win.addControl({ xtype: "ToolBar", items: [
            ...
            [{ xtype: "CheckBox", items: [{ text: "精华", value: 1 }, { text: "推荐", value: 2}], labelText: "属性：", name: "attribute"}]
            ]
    });
    ...
    var loadField = function (datatypeId) {
        $M.comm("templateManage.ajax.readDataTypeLable", { datatypeId: datatypeId }, function (json) {
            field.clear();
...
$M.templateManage.sqlLabel = function (back) {
    ...
    var loadField = function (sql) {
        $M.comm("templateManage.ajax.readSqlLable", { sql: sql }, function (json) {
            field.clear();
