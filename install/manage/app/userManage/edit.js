$M.userManage.edit = function (S) {
    ...
    if (S.id == null) {
        ...
        { xtype: "Button", name: "reset", labelText: "重置密码", text: "生成随机密码", color: 3, labelWidth: 4, width: 11,
            onClick: function () {
                $M.confirm("您确定要重置用户密码吗？", function () {
                    $M.comm("userManage.ajax.resetPassword", { id: S.id }, function (json) {
                        $M.alert("密码更改为：" + json);
                    });
                });
                return false;
        ...
        { xtype: "TextBox", name: "mobile", labelText: "手机", labelWidth: 4, width: 11, vtype: { isMobile: true} },
        { xtype: "TextBox", name: "email", labelText: "邮箱", labelWidth: 4, width: 11, vtype: { email: true} },
        { xtype: "TextBox", name: "phone", lablText: "电话", labelWidth: 4, width: 11, vtype: { isPhone: true} }
        ]);
    }
    ...
    $M.comm("userManage.ajax.readRole", null, readRole);
    if (S.id != null) {
        $M.comm("userManage.ajax.getUser", { id: S.id }, function (json) {
