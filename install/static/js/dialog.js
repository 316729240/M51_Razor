...
$M.progressDialog = function (S) {
    ...
    T.val = function (value) {
        label.html("<h2>" + (S.text ? S.text : "") + parseInt(value) + "%</h2>");
        p1.val(value);
    };
    return T;
};
$M.dialog = function (S) {
    ...
    var rdata = null;
    var footer = [
            { xtype: "Button", name: "enter", text: "确 定", type: "submit", color: 2, size: 2 },
...
$M.dialog.deleteModule = function (moduleId, type, back) {
    ...
    $M.comm("moduleDel", { moduleId: moduleId, classId: type ? moduleId : 7 }, function (json) {
        ...
        var moduleDel = function () {
            $M.comm("moduleDel", { moduleId: moduleId, classId: ids[index][0], type: 1 }, function (json) {
                index++;
