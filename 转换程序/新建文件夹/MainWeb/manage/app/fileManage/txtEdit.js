$M.fileManage.txtEdit = function (S) {
    var tab = mainTab.addItem({ text: "文本编辑器", closeButton: true, onClose: function () { $.dataManage = null; } });

    var toolBar = tab.addControl({
        xtype: "ToolBar", items: [[
            { ico: "fa-floppy-o", text: "保存", name: "saveButton",
                onClick: function () {
                    $M.comm("fileManage.ajax.saveFile", { path: S.path, fileName: S.fileName, encoding: encoding.val(), content: editBox.val() }, function (json) {
                        $M.alert("保存成功！");
                        if(S.back)S.back();
                    });
                }
    ...
    var editBox = tab.addControl({ xtype: "Editor", name: "content", sourceMode: true, style: { height: 300 }, dock: 2 });
    tab.focus();
    if (S.newfile != true) {
        $M.comm("fileManage.ajax.getFile", { path: S.path, fileName: S.fileName }, function (json) {
