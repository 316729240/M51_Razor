...
$M.system.dataManage = function (S) {
    ...
    if ($.dataManage) {
        ...
        $.dataManage._dataTypeId = S.dataTypeId;
        ...
        var toolBar = item.addControl({
            ...
            items: [
                {
                    xtype: "ButtonCheckGroup", items: [{ tip: "已审核", value: 0, ico: "fa-check-square-o" }, { tip: "未审核", value: 1, ico: "fa-exclamation-circle" }, { tip: "回收站", value: 2, ico: "fa-recycle"}], value: 0,
                ...
                [{ xtype: "Button", tip: "属性设置", name: "attrSetButton", ico: " fa-cog", menu: [
		            { text: "审核" },
        ...
        var moveData = function () {
            ...
            $M.dialog.selectColumn(item.moduleId, function (json) {
                if (item.classId == json.id) { $M.alert("目标栏目不能为当前栏目"); return; }
        ...
        var addData = function () {
            ...
            $M.app.openEdit(22192428132, { dataId: null, classId: item._classId, back: reload });
        };
        var editData = function () {
            if (grid.selectedRows.length == 0) return;
            $M.app.openEdit(22192428132, { dataId: grid.selectedRows[0].cells[0].val(), back: reload });
        };
        ...
        grid = item.addControl({
            ...
            onRowCommand: function (sender, e) {
                if (e.commandName == "link") {
                    $M.app.openEdit(22192428132, { classId: item.classId, dataId: sender.rows[e.y].cells[0].val() });
                }
            },
        ...
        var loadPage = function (pageNo, orderByName, sortDirection, type) {
            ...
            //extensionMenu = $M.loadInterface(_p, S.dataTypeId, function (name) {
            //    $M.app.call(name, { moduleId: item._moduleId,classId:item._classId,type:item._type,searchField:item_searchField,keyword:item._keyword });
            //});
