$M.templateManage.backupRestore = function (S) {
    var win = new $M.dialog({
        ...
        onClose: function (sender, e) {
            if (sender.dialogResult == $M.dialogResult.ok) {
                ...
                $M.comm("templateManage.ajax.readBackup", { id: id }, function (json) {
    ...
    $M.comm("templateManage.ajax.readBackupList", { u_webFAid: S.u_webFAid, classId: S.classId, u_type: S.u_type, u_defaultFlag: S.u_defaultFlag, u_datatypeId: S.u_datatypeId, title: S.title }, function (json) {
        grid.addRow(json);
    });
    win.show();
};