...
$M.Control= {
    ...
    $M.ajax = function (url, data, back, errback) {
        ...
        //data.append('upload_file', "aaa");
        var flag = false;
        if (data) {
            for (var o in data) {
                if (data[o]!=null && typeof (data[o]) == "object") flag = true;
        ...
        this.addControl = function (S2) {
            ...
            if ($.isArray(listC)) {
                for (var i = 0; i < listC.length; i++) {
                    T.controls[T.controls.length] = listC[i];
                }
            } else {
                T.controls[T.controls.length] = listC;
            }
            return listC;
            ...
            var addItem = function (S3) {
                ...
                if (S3.labelWidth) {
                    ...
                    if (S3.xtype) {
                        ...
                        for (var i = 0; i < S3.items.length; i++) add(line2, S3.items[i]);
                    }
                } else {
                    obj = add(line, S3);
                }
...
$M.Control["Switch"] = function (BoxID, S, CID) {
    ...
    if (CID != null) A = CID;
        ...
        if (S.textOff == null) S.textOn = "OFF";
        ...
        html += '<div class="switch has-switch ' + size + '">';
        html += '	<div class="switch-animate switch-off"><input type="checkbox" name="' + S.name + '" value=1 >';
        html += '        <span class="switch-left ' + color + ' ' + size + '">' + S.textOn + '</span><label class="' + size + '">&nbsp;</label><span class="switch-right ' + size + '">' + S.textOff + '</span>';
        html += '    </div>';
        html += '</div>';
        var B = $(html).appendTo(BoxID);
        var button = B.find(".switch-animate");
        var checkbox = B.find("input");
        B.click(function () {
            T.val(!S.value);
            if (S.onChanage) S.onChanage(T,null);
        });
    ...
    T.val = function (value) {
        if (value != null) {
            if (value) {
                button.attr({ "class": "switch-animate switch-on" });
                checkbox.attr("checked", true);
                S.value = true;
            } else {
                button.attr({ "class": "switch-animate switch-off" });
                S.value = false;
...
$M.Control["SelectBox"] = function (BoxID, S, CID) {
    ...
    if (CID != null) A = CID;
        ...
        A = $("<select class=\"\" ></select>").appendTo(BoxID);
    }
...
$M.Control["Button"] = function (BoxID, S, CID) {
    ...
    A.click(function () {
        var f = true;
        if (S.onClick) { S.onClick(); f = false; }
        return f;
    });
    if (T.menu) {
...
$M.Control["TextBox"] = function (BoxID, S, CID) {
    ...
    if (S.vtype) {
        if (typeof (S.vtype) == "string") { eval("S.vtype=" + S.vtype); }
        B.attr(S.vtype);
    }
    T.css = function (json) {
        B.css(json);
...
$M.Control["DateRangePickerInput"] = function (BoxID, S) {
    var A = $("<div class=\"input-group input-daterange input-group-" + $M.Control.Constant.sizeCss[S.size == null ? 1 : S.size] + "\" ></div>").appendTo(BoxID);
    var input = $("<input name=" + S.name + " type=hidden >");
    ...
    var value = new Date();
    var p1 = A.addControl({ xtype: "DatePickerInput", value: v1, toValue: v2 });
    $("<span class=\"input-group-btn add-on\">to</span>").appendTo(A);

    var p2 = A.addControl({ xtype: "DatePickerInput", value: v2, fromValue: v1 });
    var menu = A.addControl({ xtype: "Menu", onItemClicked: function (sender, e) {
        var day = new Date();
        switch (e.attr("val")) {
            case 0:
                p1.val(day);
    ...
    }, items: [{ text: "今天", val: 0 }, { text: "咋天", val: 1 }, { text: "本周", val: 2 }, { text: "本月", val: 3}]
    });
...
$M.Control["DatePickerInput"] = function (BoxID, S) {
    var A = null;
    ...
    var input = $("<input class=\"form-control input-" + $M.Control.Constant.sizeCss[S.size == null ? 1 : S.size] + "\" name=\"" + S.name + "\">").appendTo(A);
    ...
    var picker = popover.addControl({ xtype: "DatePicker", value: S.value, format: S.format, minValue: S.minValue, maxValue: S.maxValue, toValue: S.toValue, fromValue: S.fromValue, onChange: function () {
        popover.close();
        T.val(picker.val());
    }
    });
...
$M.Control["DatePicker"] = function (BoxID, S) {
    ...
    var showDay = function () {
	    ...
	    "<th colspan=\"5\" class=\"switch\">" + year + "年 " + month + "月</th>" +
        ...
        A.find(".switch").click(openMonthSwich);
    ...
    monthHtml += "<th colspan=\"5\" class=\"switch\">" + year + "年</th>";
    ...
    var showYearList = function () {
        ...
        monthHtml += "<thead><tr>";
        monthHtml += "<th class=\"prev\"><i class=\"glyphicon glyphicon-arrow-left\"></i></th>";
        monthHtml += "<th colspan=\"5\" class=\"switch\">" + tempyear + "年-" + (tempyear + 10) + "年</th>";
        monthHtml += "<th class=\"next\"><i class=\"glyphicon glyphicon-arrow-right\"></i></th>";
        monthHtml += "</tr></thead><tr>";
        monthHtml += "<td colspan=\"7\">";
...
$M.Control["Menu"] = function (BoxID, S) {
    ...
    T.open = function (x, y, obj) {
        ...
        if (y + height > bodyHeight) y = bodyHeight - height;
...
$M.Control["GridView"] = function (BoxID, S) {
    var T = this;
    var A = $("<div class=\"M5_GridView\"  tabindex=\"-1\"></div>").appendTo(BoxID);
    ...
    T.tableRow = function (S2) {
        ...
        var tableCell = function (S3) {
            ...
            T3.val(S3.value);
            S3.dom.click(function () {
                if (S.columns[S3.cellIndex].xtype && S.columns[S3.cellIndex].xtype != "CheckBox") {
                    var xy = td.offset();
                    foucsObj.control = A.addControl({
                        xtype: S.columns[S3.cellIndex].xtype,
                        style: { position: "fixed", zIndex: $M.zIndex++, left: xy.left + "px", top: xy.top + "px", width: td.outerWidth() + "px", height: td.outerHeight() + "px" },
                        onBlur: focusBlur
                    });
                    foucsObj.cell = T3;
...
$M.Control["Notify"] = function (BoxID, S) {
    ...
    var item = function (S2) {
        ...
        con.html((S2.ico != "" ? "<i class='fa " + S2.ico + "'/> " : "") + S2.text);
        T2.close = function () {
